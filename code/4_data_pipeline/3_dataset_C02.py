...
columns = [
    ...
    'Label', 'X-Coord', 'Y-Coord', 'B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7',
...
for tif_filename in os.listdir(tif_folder):
    ...
    # Extract pivot_id, landsat_version, year, month, and day from the new filename format
    parts = tif_filename.split('_')
    pivot_id = parts[1]
    # landsat_name = parts[2]  # OLD line, do not remove, just comment
    landsat_version = parts[2]   # Renamed to match Step 2
    year, month, day = parts[3].split('-')
    day = day.split('.')[0]

    # Build the same 5-element key used in Step 2
    key = (pivot_id, landsat_version, year, month, day)
    ...
    with rasterio.open(tif_file_path) as src:
        ...
        if annotation_list:
            for ann_item in annotation_list:
                for result in ann_item.get('result', []):
                    if result['type'] == 'keypointlabels':
                        ...
                        row = [
                            tif_id, tif_filename, landsat_version, year, month, day,
                            x_percent, y_percent, label, x, y
                        ] + band_data
                        data.append(row)
